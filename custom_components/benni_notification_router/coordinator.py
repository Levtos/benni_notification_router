"""Event manager / coordinator for benni_notification_router."""
...
from .const import (
    BIO_STATES, ACTIVITY_STATES, PRESENCE_STATES,
    ...
    DEFAULT_COOLDOWNS, DEFAULT_RATE_LIMIT, DOMAIN,
    EVENT_CLASSES, EVENT_ROUTED,
    OPT_COOLDOWNS, OPT_PRIVATE_TIME_BEHAVIOR, OPT_QUIET_HOURS_END,
    OPT_QUIET_HOURS_START, OPT_RATE_LIMIT, OPT_SEVERITY_MAP,
    OPT_SLEEP_BEHAVIOR,
    ROUTE_LIGHT, ROUTE_MEDIA, ROUTE_PERSISTENT, ROUTE_PUSH,
...
class NotificationRouter:
    """Holds runtime state and exposes routing entrypoints."""
    ...
    async def async_route(
        ...
        # Apply per-class severity override from options
        ...
        if suppression_reasons:
            decision.routes = [r for r in decision.routes if r in ("dashboard_event", "bus_only")]
        ...
        for route in decision.routes:
            try:
                if route == ROUTE_PUSH and targets:
                    for svc in targets:
                        # svc expected like "notify.mobile_app_foo"
                        if "." not in svc:
                            continue
                    ...
                    await self.hass.services.async_call(
                        ...
                        {
                            "title": decision.title or "Benni Router",
                            "message": decision.message,
                            "notification_id": ev.dedupe_key or f"{ev.event_type}_{int(time.time())}",
                        },
                        blocking=False,
                    )
                elif route == ROUTE_LIGHT and light_script:
                    await self._call_script(light_script, {"severity": decision.severity, "event_type": ev.event_type})
                elif route == ROUTE_MEDIA and media_script:
                    await self._call_script(media_script, {"title": decision.title, "message": decision.message})
    ...
    async def _call_script(self, entity_id: str, variables: dict[str, Any]) -> None:
        ...
        if domain == "script":
            await self.hass.services.async_call(
                "script", obj, variables, blocking=False,
            )
        else:
            await self.hass.services.async_call(
